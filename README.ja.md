...
- **非表示だが気づけるカード**（`exclude_labels`）: 指定ラベル（「やらない」等）の
  ...
  薄色表示して確認できます
- **セッションゴースト**: セッションのタブが生きているのに検索から外れた課題
  （完了して期間切れ・アサイン変更など）は薄色のカードとして盤面に残り、
  `Enter`（タブへ移動）と `o`（ブラウザで開く）がそのまま使えます。タブを
  閉じれば次の更新で消えます
...
タブアクションです（herdr の右クリックメニューには組み込み項目しか並ばないため、
実行手段はキーバインドか `herdr plugin action invoke open-issue --plugin jira-board`
です）。課題キーは
